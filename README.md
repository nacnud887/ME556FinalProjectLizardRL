...
### 1) Lizard Robot Code
Under source > isaaclab_assets_isaaclab_assets > robots, I saved lizard.py

### 2) Reinforcement Learning Code
In source>  isaaclab_tasks > isaaclab_tasks > manager_based > locomotion > velocity > config, I made a folder called lizard and put in __init__.py, flat_env_cfg.py, and rough_env_cfg.py.
In another folder under that, titled agents, I saved rsl_rl_ppo_cfg.py.

### 3) WandB Tracking
This is saved outside of the Isaac Repo, and in my system's AppData for the Conda Environment.
In AppData\Local\miniconda3\envs\env_isaaclab\Lib\site-packages\rsl_rl\utils\, I saved wandb_utils.py

### 4) Export Joint Angles
In scripts > reinforcement_learning > rsl_rl, I saved play.py
This exported LizardRL_Radians_12-11-2025.csv

## MATLAB Code for Hardware

### 1) Code Setup
I adapted the code that came with the Dynamixel SDK to be usable for my case.
I named this file LizardCodeRL.m
In the same folder I saved LizardRL_Radians_12-11-2025.csv, so that MATLAB could Read it.

### 2) Code Output
The output from the code is saved as RLTrialLog.txt. It lists the encoder values of each servo from 0 to 4095.
